...
export default function AdminUsers() {
  const { user } = useAuth();
  const { data: res, refetch } = useApiQuery(() => supabaseApi.admin.getAllUsers());
  const users = res?.users || [];

  if (!user || user.role !== "ADMIN") return null;

  const handleRoleChange = async (userId: string, role: string) => {
    await supabaseApi.admin.updateRole(userId, role);
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            ...
            <tbody className="divide-y">
              {users?.map((u: any) => (
                <tr key={u.id || u._id} className="hover:bg-white">
                  ...
                  <td className="px-6 py-4"><select value={u.role} onChange={(e) => handleRoleChange(u.id || u._id, e.target.value)} className="text-sm border rounded px-2 py-1"><option value="CUSTOMER">Cliente</option><option value="VENDOR">Vendedor</option><option value="ADMIN">Admin</option></select></td>
