...
const statusColors: Record<string, string> = { PENDING: "bg-[#FFE5EA] text-[#CC0033]", PAID: "bg-blue-100 text-blue-800", PROCESSING: "bg-purple-100 text-purple-800", SHIPPED: "bg-indigo-100 text-indigo-800", DELIVERED: "bg-green-100 text-green-800", CANCELLED: "bg-red-100 text-red-800" };
...
export default function Orders() {
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8">
      ...
      {orders === undefined ? <p className="text-uniko-blue/70">Cargando...</p> : orders.length === 0 ? (
          ...
          {orders.map((order: any) => (
            <div key={order._id} className="bg-white rounded-xl shadow-sm p-6">
              <div className="flex items-start justify-between mb-4">
                <div><p className="font-semibold">{order.orderNumber}</p><p className="text-sm text-uniko-blue/70">{new Date(order._creationTime).toLocaleDateString("es-DO")}</p></div>
              ...
              <div className="space-y-2 mb-4">
                {order.items?.map((item: any) => (
                  <div key={item._id} className="flex items-center gap-3">
                    ...
                    <div className="flex-1 min-w-0"><p className="text-sm font-medium truncate">{item.name}</p><p className="text-xs text-uniko-blue/70">x{item.quantity}</p></div>
