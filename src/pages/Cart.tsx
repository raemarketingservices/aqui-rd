          ...
          {cart.items.map((item: any) => (
            <div key={item._id} className="bg-white rounded-xl shadow-sm p-4 flex flex-col sm:flex-row gap-4">
              ...
              <div className="flex-1 min-w-0">
                <Link to={`/producto/${item.productId}`} className="font-semibold text-uniko-blue hover:text-uniko-blue line-clamp-1">{item.product?.name}</Link>
                <p className="text-sm text-uniko-blue/70">{item.product?.vendor?.businessName}</p>
                <p className="text-lg font-bold text-uniko-dark mt-1">RD${(item.product ? item.product.price / 100 : 0).toLocaleString()}</p>
                <div className="flex items-center justify-between mt-2">
                  <div className="flex items-center gap-2">
                    <button onClick={() => updateQuantity({ cartItemId: item._id, quantity: item.quantity - 1 })} className="w-10 h-10 rounded border flex items-center justify-center hover:bg-white"><FiMinus size={14} /></button>
                    <span className="w-8 text-center font-medium">{item.quantity}</span>
                    <button onClick={() => updateQuantity({ cartItemId: item._id, quantity: item.quantity + 1 })} className="w-10 h-10 rounded border flex items-center justify-center hover:bg-white"><FiPlus size={14} /></button>
                  </div>
        ...
        <div className="bg-white rounded-xl shadow-md p-6 h-fit sticky top-24">
          ...
          <div className="space-y-3 text-sm">
            ...
            {taxLines.length > 0 ? taxLines.map((t, i) => (
              <div key={i} className="flex justify-between"><span className="text-uniko-blue">{t.name} ({t.rate}%)</span><span className="font-medium">RD$${t.amount.toLocaleString(undefined, { minimumFractionDigits: 2 })}</span></div>
            )) : <div className="flex justify-between"><span className="text-uniko-blue">Impuestos</span><span className="font-medium">No aplica</span></div>}
